...
var Grid = {
    ...
    grid_size: 50,
    min_grid_size:14,
    grid_meter: 10, //number of grid squares per meter
...
var InputController=function(){
    ...
    $('#wood-thickness-input').change(function () {
        var wood_thickness = parseFloat($(this).val());
        if (!isNaN(wood_thickness)) { //to make sure the input is valid (is an integer and less than 10)
            EntityController.wood_thickness = wood_thickness;
        }
    });
        ...
        if (ModeController.mode === 'add_node' && !ModeController.simulation) {
            ...
            if (event.target && event.target.type === 'node') { //if a node has been clicked on
                if (!ModeController.new_member.start_node) { //if the member's start has not been determined yet
                    ...
                    ModeController.new_member = new Member({thickness: EntityController.wood_thickness}); //create a new member while leaving the old one in the canvas
                    if(event.e.shiftKey) {
                        ModeController.new_member.set({ //position the start of the member to be at the center of the node
                            x1: event.target.left,
                            y1: event.target.top,
...
var Member = fabric.util.createClass(fabric.Line, {
    ...
    initialize: function(options) {
        ...
        var tmpThickness = options.thickness;
        this.callSuper('initialize', options);

        this.set({
            fill: 'blue',
            stroke: 'hsla(243, 0%,50%, 1)',
            strokeWidth: 40 * (tmpThickness || 1),
            strokeLineJoin : "round",
            ...
            mthickness: tmpThickness || 0.125 //so we have different thicknesses
        });
    },
...
var ModeController={
	...
	add_member_mode:function(){
		...
		if(this.mode!=='add_member'){ //if not already in add-member mode
			...
			this.new_member=new Member({thickness: EntityController.wood_thickness});
...
var ResizeController={
	...
	resizeCanvas: function(){
		if(ResizeController.canvas){
			ResizeController.canvas.setHeight($(window).height() + 1000);
